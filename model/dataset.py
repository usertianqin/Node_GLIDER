...
from transleted_both_X import transleted_X
...
def load_twitch_dataset(data_dir, lang):
    ...
    new_node_feat =torch.tensor( transleted_X(node_feat))
...
def load_good_dataset(data_dir, dataname='', sub_dataname=0):

    if dataname == 'webKB':
        datatype = 'university'


    A = np.load(data_dir + '/' + dataname + '_result/' + datatype + '/' + str(sub_dataname) + '/A.npy', allow_pickle=True)
    label = np.load(data_dir + '/' +dataname + '_result/' + datatype + '/' + str(sub_dataname) + '/Y.npy', allow_pickle=True)
    features = np.load(data_dir + '/' + dataname + '_result/' + datatype + '/' + str(sub_dataname) + '/X.npy', allow_pickle=True)
    dataset = NCDataset(str(sub_dataname))
    for i in range (len(label)):
        if label[i] is None:
            print("label is None:", i)
    ...
    dataset.graph = {'edge_index': edge_index,
                     ...
                     'node_feat': new_node_feat,
                     'num_nodes': num_nodes}
    dataset.label = torch.tensor(label, dtype=torch.int64)
    return dataset
