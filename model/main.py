...
from dataset import load_nc_dataset, load_good_dataset
...
def get_dataset(dataset, sub_dataset=None):
    ...
    if dataset == 'twitch-e':
        ...
        dataset = load_good_dataset(args.data_dir, 'webKB', sub_dataset)
    else:
...
if args.dataset == 'twitch-e':
    ...
    tr_subs, val_subs, te_subs = ['ES', 'PTBR', 'TW', 'DE', 'RU', 'ES'], ['DE'], ['FR']
    datasets_tr =  [get_dataset(dataset='twitch-e', sub_dataset=tr_subs[i]) for i in range(len(tr_subs))]
    print("training transleted finished!!!")
    datasets_val = [get_dataset(dataset='twitch-e', sub_dataset=val_subs[0])]
    print("val transleted finished!!!")
    datasets_te = [get_dataset(dataset='twitch-e', sub_dataset=te_subs[0])]
    print("testing transleted finished!!!")
    ...
    print("training transleted finished!!!")
    datasets_val = [get_dataset(dataset='fb100', sub_dataset=val_subs[0])]
    print("val transleted finished!!!")
    ...
    for i in range(len(tr_subs)):
        dataset_tr = get_dataset(dataset='webKB', sub_dataset=tr_subs[i])
        datasets_tr.append(dataset_tr)
    print("training transleted finished!!!")
    datasets_val = [get_dataset(dataset='webKB', sub_dataset=2)]
    print("val transleted finished!!!")
    datasets_te = [get_dataset(dataset='webKB', sub_dataset=1)]
...
if args.dataset == 'fb100':
    ...
    for i in range(len(te_subs)):
        ...
        print(f"Test num nodes {dataset_te.n} | num classes {dataset_te.c} | num node feats {dataset_te.d}")
else:
    for i in range(len(tr_subs)):
        dataset_tr = datasets_tr[i]
        print(f"Train num nodes {dataset_tr.n} | num classes {dataset_tr.c} | num node feats {dataset_tr.d}")
    ...
    for epoch in range(args.epochs):
        ...
        if args.method == 'IDDG':
            if args.dataset == 'twitch-e':
                ...
                beta = 10 * args.beta * epoch / args.epochs + 1 * args.beta * (1 - epoch / args.epochs)
